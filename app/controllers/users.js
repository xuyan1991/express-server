...
class UsersController {
  ...
  async currentUser (req, res) {
    try {
      let userId = auth.verifyToken(req.headers.token)
      const result = await Services.users.detail(userId.userId)
      res.sendOk(result)
    } catch (error) {
      const errorRes = resHandler.getErrorRes('TOKEN_IS_INVALID')
      res.sendErr(errorRes)
    }
  }
